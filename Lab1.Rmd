...
```{r}
ggplot(data = pumpkins) + geom_point(aes(x = day, y = price))
```
...
new_pumpkins |> group_by(month) |> summarize(mean(price))
```
...
new_pumpkins |> group_by(month) |> summarize(mean_price = mean(price)) |> 
  ggplot() + geom_col(aes(x = month, y = mean_price)) + labs(title = "Mean Pumpkin Price by Month", y = "mean price $")
```
...
**9 cities are represented in the dataset**
...
**Variety is most highly correlated with price, which makes sense because price would vary a lot dependent on the variety.**
...
**The relationship between price and package does not appear to follow a linear relationship, rather price tends to be grouped based on package size.**
